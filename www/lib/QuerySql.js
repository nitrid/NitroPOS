var QuerySql = 
{
    ...
    CmbKasaGetir : 
    {
        query : "SELECT kas_kod AS KODU,kas_isim AS ADI FROM KASALAR " ,
    }, 
    CmbBankaGetir : 
    {
    ...
    CariListeGetir : 
    {
        query : "SELECT cari_kod AS KODU, " +
                ...
                "ISNULL(ROUND((SELECT dbo.fn_CariHesapBakiye(0,cari_baglanti_tipi,cari_kod,'','',0,cari_doviz_cinsi,0,0,0,0)) + (SELECT SUM(FIYAT) - (SELECT SUM(TUTAR) FROM TERP_POS_TAHSILAT WHERE MKODU = cari_kod) FROM TERP_POS_SATIS WHERE MKODU = cari_kod),0),0) AS BAKIYE, " +
    ...
    MaxPosTahSira : 
    {
        query: "SELECT ISNULL(MAX(SIRA),0) + 1 AS MAXEVRSIRA FROM TERP_POS_TAHSILAT " +
                "WHERE SUBE = @SUBE AND SERI = @SERI AND EVRAKTIP = @TIP " ,
        param : ['SUBE','SERI','TIP'],
        type : ['int','string|25','int']
    },
    ...
    {
        query: "UPDATE [dbo].[TERP_POS_SATIS] SET [MIKTAR] = (MIKTAR * @MIKTAR),[ISKONTO] = @ISKONTO WHERE RECID = @RECID",
        param: ['MIKTAR','ISKONTO','RECID'],
        type:  ['float','float','int']
    },
    PosSatisBelgeIptal : 
    ...
    PluInsert :
    {
        query : "INSERT [dbo].[TERP_POS_PLU] " +
                ...
                "[GRUP] " +
                ") VALUES ( " +
                "@KULLANICI          --<KULLANICI, nvarchar(25),> \n" +
                ",@KODU			--<KODU, nvarchar(25),> \n" +
                ",@ADI			--<ADI, nvarchar(25),> \n" +
                ",@GRUP			--<GRUP, nvarchar(25),> \n" +
                ")",
        param  :['KULLANICI:string|25','KODU:string|25','ADI:string|25','GRUP:string|25']
    },
    PluDelete :
    {
    ...
    PluTanimListeGetir : 
    {
        query : "SELECT RECID,KULLANICI,ISNULL((SELECT sto_isim FROM STOKLAR WHERE sto_kod = BARKOD_TANIMLARI.bar_stokkodu),'') AS STOKADI,ADI AS PLUADI,GRUP,bar_kodu AS KODU " +
    ...
    PluGrupListeGetir :
    {
        query : "SELECT GRUP,COUNT(ADI) as PLUSAYI FROM TERP_POS_PLU WHERE KULLANICI = @KULLANICI GROUP BY GRUP ORDER BY GRUP",
        param :['KULLANICI:string|25']
    },
    PosSatisIskonto :
    {
