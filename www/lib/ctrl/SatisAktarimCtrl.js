    ...
    $scope.BtnGetir = function()
    {
        ...
        var TmpQuery = 
        {
            ...
            query:  "SELECT " +
                    ...
                    "GROUP BY SKODU,SUBE,SERI,SIRA,TARIH,MKODU,TIP ORDER BY CONVERT(DATETIME,TARIH),SUBE,MKODU,SERI,SIRA,TIP ASC",
    ...
    $scope.BtnAktar = async function()
    {
        ...
        for(i = 0;i < $scope.IslemListe.length;i++)
        {
            if($scope.IslemListe[i].SUBE != TmpSube || $scope.IslemListe[i].TARIH != TmpTarih || $scope.IslemListe[i].MKODU != TmpCariKodu || $scope.IslemListe[i].TIP != TmpTip)
            {
                ...
                if($scope.IslemListe[i].MKODU == UserParam.PosSatis.Cari)
                {
                ...
                {
                    TmpEvrSira = $scope.IslemListe[i].SIRA;
                }
